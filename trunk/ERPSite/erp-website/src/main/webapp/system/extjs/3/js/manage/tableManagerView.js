				...
				buttons: [{
					...
					listeners:{
						'click':function(_button){
							var formPanel = _button.ownerCt.ownerCt.items.get(0).get(0).items.get(0);
							var gridPanel = _button.ownerCt.ownerCt.items.get(0).get(1).items.get(0);
														
							formPanel.getForm().isValid();
							// GridPanel 的数据需要一个一个严重组合
							//iWrite(gridPanel);
						}
					}
                },{
